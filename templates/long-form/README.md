...
> **Status:** 6 templates available — [`standard/`](./standard/) is the canonical generic baseline; [`anthropic/`](./anthropic/) is the Anthropic dark-stage variant for Claude / Anthropic article-response and deep-dive content; [`google/`](./google/) is the Google brand cinematic variant for Cloud Next / I/O / Gemini / Workspace / TPU content; [`claude-code-version/`](./claude-code-version/) is the Claude Code release-update variant; [`hostinger-sponsored/`](./hostinger-sponsored/) is the Hostinger sponsorship cadence (single-file 53.5s tutorial); [`dynamous-slides/`](./dynamous-slides/) is the classic Dynamous deep-navy + blue workshop deck.
...
| [`google/`](./google/) | Google brand cinematic — `#0C0C0E` near-black + four Google-color corner glows + canonical four-color rotation (blue `#4285F4` / red `#EA4335` / yellow `#FBBC04` / green `#34A853`) + Google Cloud cyan `#00BCD4` reserved for architecture-stack l5 + progress-bar gradient | Google Cloud Next / I/O recaps, Gemini / Gemini 3 / Gemma launches, Workspace AI explainers, Android / Pixel software releases, TPU / DeepMind / Google AI walkthroughs, any article response to a `google.com` / `cloud.google.com` / `deepmind.google` post. 8 scene archetypes (same 8 as `standard/`, re-painted Google) + Google wordmark top-right watermark (180×180, `top:-20, right:30` per V2 Remotion reference) + Inter type. Authored as a palette-swap fork of `standard/`. |
