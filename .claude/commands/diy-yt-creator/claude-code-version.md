...
3. **Draft `videos/<slug>/youtube-description.md`** in the rule's prescribed order: hook (keyword-front-loaded, primary keywords in first 200 chars) → Dynamous CTA in `----` separators → Key Changes bullets → `Chapters` header line + chapter list → Resources (validated URLs with keyword-rich anchor text) → primary CTA (`$ claude update`) → polarizing debate question (must match script's final line) → 15-25 hashtags drawn from the vidIQ shortlist.
4. **Validate every URL** with `WebFetch` — replace 404 / redirect chains with working URLs.
