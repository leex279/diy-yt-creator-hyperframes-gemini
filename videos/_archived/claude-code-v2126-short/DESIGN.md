...
A GitHub-dark stage tuned for Claude Code releases on vertical canvas. Off-white type sits on `#0D1117` (the GitHub dark surface). The accent triad is Claude Code's own palette: cyan-blue (`#58A6FF`) for primary / version slam, purple (`#A371F7`) for tertiary, green (`#3FB950`) for success / improved. Orange (`#F78166`) handles warnings and the Subscribe pulse — not the hero. A persistent VersionBranding overlay (Anthropic + Claude Code logos top-right at 0.7 opacity, repo URL + version string bottom-center) frames every phase as part of the Claude Code visual identity. The CTA is a macOS terminal block showing `$ claude update` — the canonical close for Claude Code release content. Otherwise reads exactly like the anthropic shorts: dark-stage, percussive springs, scroll-stop hero, no music.
...
| Accent — cyan-blue | `--accent-1` | `#58A6FF` | Primary lead, version slam (Phase 1), stat-pill 1 (features), highlight-card 1, terminal flag |
...
**Accent rotation rule:** within one video, each phase pins one accent. Phase 1 leads cyan; Phase 2 stat pills rotate cyan/purple/green left-to-right; Phase 3 highlight cards rotate cyan/purple/green top-to-bottom; Phase 4 CTA uses cyan + green (terminal) and orange (Subscribe pulse). Orange is NEVER on a highlight card or a stat pill — that's the warn / brand pulse channel.
...
| Role | Size | Notes |
|---|---|---|
| Hero version slam (Phase 1) | 200px | Cyan-blue with cyan glow; tabular-nums for `vN.M.PPP` |
| Hero pre-line (Phase 1) | 64px | "Version" or release adjective; -1px lower than anthropic's 72px to give the slam more visual real estate |
| Headline (Phase 2) | 56px | -8px from anthropic's 64px to fit "The receipts." with the 3-pill row below |
| Stat number (Phase 2 pills) | 140px | -40px from anthropic's 180px because there are 3 pills horizontally instead of 2 — width-bounded |
| Stat label (Phase 2) | 38px | +8px from anthropic's 30px because the 3-pill layout has wider per-pill copy room than 2-pill |
| Highlight card title (Phase 3) | 38px | -2px from anthropic's 40px (tighter row gap on numbered cards) |
| Highlight card sub (Phase 3) | 26px | -2px from anthropic's 28px |
| Number badge (Phase 3) | 48px | mono 900, solid accent fill, near-black `color: var(--bg)` for AAA contrast — replaces anthropic's date chip |
| Terminal content (Phase 4) | 38px | mono; `$` prompt green, flag (`update`) cyan-blue |
| Subscribe pill (Phase 4) | 38px | sans 700; orange arrow at 48px |
| Section overline | 36px | mono 700, UPPERCASE, letter-spacing 6px — same as anthropic |
| Caption pill (Phase 1) | 30px | mono 600, letter-spacing 2px — same as anthropic |
| VersionBranding URL | 22px | mono, secondary text, letter-spacing 1px — bottom-center for vertical canvas (long-form bottom-right) |

**Tabular numerals on stat numbers and the version slam:** `font-variant-numeric: tabular-nums lining-nums` so digits don't jitter across springs.
...
**Inline shake** on the Phase-1 version slam (4 ticks ±5px, 40ms each). One shake per phase max.

**Subscribe pulse** (Phase-4 CTA, variant-specific): `boxShadow` glow tween at `rgba(247, 129, 102, 0.40)`, `yoyo: true, repeat: 4`, 0.7s per half-cycle = ~3.5s span. Finite repeat — never `repeat: -1`.
...
- **Entrance:** `tl.from("#p4-terminal", { y: 24, scale: 0.96, opacity: 0, duration: 0.6, ease: "back.out(1.4)", immediateRender: false }, P4 + 0.5)`.
...
|---|---|---|
| `impact-slam` | Phase-1 version slam reveal | 0.15 |
| `scale-slam` | Phase-2 stat-pill entrances (one per pill or hero pill only) | 0.15 |
| `screen-shake` | Phase-1 inline shake | 0.11 |
| `cinematic-whoosh` | Phase / scene change (transitions T1, T2, T3) | 0.11 |
| `spring-pop` | Highlight card or chip entrance | 0.11 |
| `pop` | Number badge entrance | 0.10 |
