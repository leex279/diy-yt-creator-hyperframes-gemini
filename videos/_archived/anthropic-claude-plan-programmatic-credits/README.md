...
|---|---|---|
| 1a — Thumbnail intro | brand lockup + 160px topic slam + receipt line (static from t=0) | **first-frame thumbnail** — YouTube auto-picks this as the tile image |
| 1b — Hero slam | overline + secondary line + 240px slam word + caption pill (fades in at ~2.5s) | scroll-stop hook / pain pivot |
...
**Phase 1 note:** Both beat 1a (topic lockup, t=0) and the final frame (Phase 4 or a dedicated `#phase-thumb`) must satisfy the 5-element thumbnail checklist per [`.claude/rules/shorts-thumbnail-frames.md`](../../../.claude/rules/shorts-thumbnail-frames.md). YouTube evaluates BOTH ends — the first frame for the upload thumbnail, the last frame for loop-pause + screenshot sharing.
