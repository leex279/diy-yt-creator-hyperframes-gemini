...
- **OpenAI shape backdrop** — 12 deterministic SVG shape instances at 6% opacity (spirograph hexagon · Daybreak sunrise arcs · hex/network nodes). Replaces the Google template's particle dots; brand-coherent and reorders on every phase + sub-slide transition paired with `cinematic-whoosh` SFX.
- **Inner safe-area frame** (rounded hairline border)

### Hero figure slot (opt-in — Phase 1 screenshot pattern)

When the topic is sourced from a tweet, blog post, or UI screenshot, replace the Phase 1 `<p class="subhead">` with a `<figure class="hero-figure">` block:

```html
<figure class="hero-figure" id="p1-figure" style="width: 920px;">
  <img src="assets/x-post-source.png" alt="Source post" />
  <figcaption><span class="src-handle">@source</span> · POST · DATE</figcaption>
</figure>
```

Then animate it in with a premium tilt-in:

```js
tl.from('#p1-figure', {
  y: 40, scale: 0.94, opacity: 0,
  rotateY: 8, transformPerspective: 1200,
  duration: 0.9, ease: 'power3.out'
}, 1.7);
```

The `.hero-figure` CSS (already loaded) provides a mint-glow drop shadow, rounded card border, mint top hairline, and a mono figcaption with accent-tinted `@source` handle. Drop the screenshot into `videos/<slug>/assets/` (HyperFrames rejects `../../shared` paths — local copies only).
