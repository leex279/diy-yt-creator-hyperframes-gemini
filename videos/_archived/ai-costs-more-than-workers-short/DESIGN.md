...
## Phase 1 — Thumbnail-grade intro (open-topic-then-pivot pattern)

Every Anthropic Short's Phase 1 uses the **open-topic-then-pivot** beat structure. This satisfies the first-frame requirement in [`.claude/rules/shorts-thumbnail-frames.md`](../../../.claude/rules/shorts-thumbnail-frames.md) — YouTube auto-picks the first frame as the thumbnail when none is uploaded, so t=0 must be thumbnail-grade.

### Beat 1a (0.0s → ~2.5s) — Thumbnail-grade topic lockup

The dominant visible content from frame zero. No entrance fade-in — the elements are immediately at full opacity so the very first rendered frame is thumbnail-grade.

**Required elements (all 5 from the thumbnail rule):**

1. **Brand lockup** (`#p1-brand-lockup`) — Anthropic logo (+ partner logo + divider for collab videos). Height ~80–120px. Position: upper area of content zone.
2. **Topic slam** (`#p1-topic-slam`) — the video's main topic phrase, 140–180px, Inter 900, letter-spacing -5px. Dominant element.
3. **Outcome receipt** (`#p1-ga-receipt`) — one short line: "GA · DATE · COUNT" or "N features · DATE" format, 44px mono.
4. *(Visual anchor is the brand lockup — both logo + topic together constitute the anchor)*
5. *(CTA pill is optional and rare at t=0 — the topic slam serves the scroll-stop function)*

**GSAP wiring:**
```js
// No entrance tween — lockup is at full opacity at t=0 (first-frame thumbnail).
// Fade it out when beat 1b narration arrives (~2.4s):
tl.to("#p1-topic-lockup", { opacity: 0, duration: 0.5, ease: "power2.in" }, 2.4);
```

`visual-pacing-5s.md` is **explicitly relaxed** for this 2.5s opening hold — same rationale as the terminal hold.

### Beat 1b (~2.5s → phase end) — Hook / hero slam

Original hook content (pain reveal, "WEEKS LATE", rhetorical slam, etc.) fades in as the lockup fades. Wire element entrances to `transcript.json` word anchors.

```js
tl.to("#p1-hero-content", { opacity: 1, duration: 0.5, ease: "power2.out" }, 2.5);
// Then wire #p1-pre, #p1-hero, #p1-caption to transcript word.start values.
```

**Why this order works:** Viewers who tap from the thumbnail (auto-picked from t=0 frame) see exactly what they expected. Viewers who scroll past mid-Short get the hook pivot at 2.5s. The algorithm sees a short that opens confidently with its topic.
