...
A self-contained 24-second demo composition (`index.html`) showcasing **four reusable phase archetypes** wired for Claude Code release content:

| Phase | Pattern | Use for |
|---|---|---|
| 1 — Version slam | overline ("Claude Code Update") + "Version" pre-line + 200px slam version (e.g. `v2.1.118`, cyan-blue) + caption pill | scroll-stop hook anchored on the version number |
| 2 — Stats opener | overline + headline + 3 huge stat pills (cyan / purple / green) | features / fixes / improved counts pulled from the changelog |
| 3 — Highlight cards | overline + 3 numbered cards (`01/02/03`, cyan → purple → green) | top 3 highlights of the release (no dates — number badges instead) |
| 4 — CTA | overline + `$ claude update` terminal block (macOS chrome) + Subscribe pill (Claude Code orange glow pulse) | the canonical Claude Code release-video close |
...
| Phase 1 hero target | Slam word ("DUMBER?") | Slam version (`v2.1.118`) — keeps the spring, swaps the payload |
| Phase 2 stat pills | 2 pills (orange + purple) | **3** pills (cyan / purple / green) for features / fixes / improved |
| Phase 3 cards | Dated timeline cards (Mar 4 / Mar 26 / Apr 16) | Numbered highlight cards (`01` / `02` / `03`) — no dates, top-N highlights |
| Phase 4 CTA | URL pill + Subscribe pill | macOS terminal block showing `$ claude update` + Subscribe pill (Claude Code orange pulse) |
...
#    - Phase 1: replace #p1-hero "vX.Y.Z" with the real version
#    - Phase 2: replace each .stat-num with real counts (features / fixes / improved)
#    - Phase 3: replace each .hl-title and .hl-sub with the top 3 highlights
#    - Phase 4: terminal already shows "$ claude update" — leave it
